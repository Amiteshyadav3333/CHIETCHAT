...
const ACCESS_SECRET = process.env.JWT_ACCESS_SECRET || 'podlive-default-jwt-access-secret-key-2026';
const REFRESH_SECRET = process.env.JWT_REFRESH_SECRET || 'podlive-default-jwt-refresh-secret-key-2026';

const generateTokens = (userId) => {
    const accessToken = jwt.sign({ id: userId }, ACCESS_SECRET, { expiresIn: '7d' });
...
exports.register = async (req, res) => {
    try {
        ...
        const cleanHandle = unique_handle.trim().toLowerCase().replace(/[^a-z0-9_]/g, '');

        // Check if user exists
        const existingUser = await prisma.user.findFirst({
            where: {
                OR: [{ email: cleanEmail }, { unique_handle: cleanHandle }]
            }
        });
    ...
    try {
        ...
        const user = await prisma.user.findFirst({
            where: {
                OR: [
                    { email: identifier },
                    { unique_handle: identifier }
                ]
            }
        });

        if (!user) {
            return res.status(401).json({ error: 'Invalid email/handle or password.' });
        }
...
exports.cheetchatSso = async (req, res) => {
    ...
    try {
        ...
        const candidateApis = Array.from(new Set([
            process.env.CHEETCHAT_API_URL,
        ...
        for (const api of candidateApis) {
            try {
                let res = await fetch(`${api}/api/auth/podlive-sso/verify`, {
                    method: 'POST',
                ...
                if (res.status === 404 || res.status === 405) {
                    ...
                    if (res.ok) {
                        ...
                        identity = {
                            ...
                            email: profile.user.email || `cheetchat-${profile.user.id}@sso.invalid`,
        ...
        if (!verificationResponse || !identity) {
            return res.status(401).json({ error: 'CHEETCHAT could not verify this sign-in ticket. Please log in again.' });
        }
