...
export const podlive = {
    ...
    vods: () => podliveApi.get('/api/live/vods').then(({ data }) => data),
    recording: (id) => podliveApi.get(`/api/live/${id}/recording`).then(({ data }) => data),
    ...
    start: async (id) => {
        const [{ data }, config] = await Promise.all([podliveApi.post(`/api/live/${id}/start`), podliveApi.get('/api/config').then((response) => response.data)]);
        return { ...data, livekitUrl: config.livekitUrl };
    },
    startHls: (id) => podliveApi.post(`/api/live/${id}/hls/start`).then(({ data }) => data),
    ...
    recordings: () => podliveApi.get('/api/user/recordings').then(({ data }) => data),
    ...
    removeRecording: (id) => podliveApi.delete(`/api/live/${id}`).then(({ data }) => data),
    upload: (formData, onUploadProgress) => podliveApi.post('/api/upload', formData, { onUploadProgress }).then(({ data }) => data),
};
