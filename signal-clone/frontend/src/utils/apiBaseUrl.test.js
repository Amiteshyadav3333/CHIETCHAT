...
describe('production API URL resolution', () => {
    it('replaces the retired Render hostname', () => {
        expect(resolveApiBaseUrl('https://chietchat-backend.onrender.com', true))
            .toBe('https://chietchat.onrender.com');
    });

    it('uses the production backend when the build variable is missing or unsafe', () => {
        expect(resolveApiBaseUrl('', true)).toBe('https://chietchat.onrender.com');
